...
ATTRIBUTE_NORETURN void UniversalInputStream::ioError (const char* fileOrigin, int lineOrigin,
                                                       const char* functionOrigin, std::string operation)
{
    isBroken = true;
...
unique_ptr <UniversalInputStream> UniversalInputStream::openInputStream (string fileName, RelativeInputStreamFlags flags)
{
    ...
    if (uint32_t (flags))
        throw InvalidArgumentException (__ORIGIN__,
                                        "Unknown RelativeInputStreamFlags flags remaining: '" + toString (uint32_t (flags)) +
...
ATTRIBUTE_NORETURN void FileOutputStream::ioError (const char* fileOrigin, int lineOrigin,
                                                   const char* functionOrigin, string operation)
{
    isBroken = true;
    throw InputOutputException (fileOrigin, lineOrigin, functionOrigin, getStreamDescription(), operation);
}
...
unique_ptr <FileOutputStream> FileOutputStream::openOutputStream (string fileName, RelativeOutputStreamFlags flags)
{
    ...
    if (uint32_t (flags))
        throw InvalidArgumentException (__ORIGIN__,
                                        "Unknown RelativeOutputStreamFlags flags remaining: '" + toString (uint32_t (flags)) +
                                        "' (bits " + getBitPositions (uint32_t (flags)) + ").", "flags");
